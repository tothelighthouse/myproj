...
public class PageCriteria {
	private static final Logger logger = LoggerFactory.getLogger(PageCriteria.class);
	...
	private void init() {
	  ...
	  //요청페이지의 종료페이지 계산:올림(요청페이지/한페이지에보여줄 페이지수)*한페이지에보여줄 페이지수
		...
		logger.info("endPage 계산된 값 :" + String.valueOf(this.endPage));
		logger.info("this.rc.getReqPage():" + String.valueOf(this.rc.getReqPage()));
		...
		this.finalEndPage = (int)Math.ceil((totalRec / (double)rc.getNumPerPage()));
		logger.info("totalRec 계산된 값 : "+totalRec);
		if(this.finalEndPage < this.getEndPage()  ) {
			logger.info("finalEndPage 계산된 값 : "+finalEndPage);
