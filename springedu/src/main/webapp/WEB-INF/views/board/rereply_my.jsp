		...
		xhttp.addEventListener("readystatechange",function(){
			if(this.readyState == 4 && this.status == 200){
				...
				jsObj.list.forEach( e => {
					...
					str += '        <div class="col"><span><i class="fas fa-reply rereplyBtn" title="댓글달기">댓글달기</i></span></div>';
					str += '        <div class="col pw-10">';
					str += '          <div class="row">';
					if(e.rid == l_id){
						str += '            <div class="col"><span><i class="fas fa-edit replyModifyBtn" title="수정하기">수정</i></span></div>	';
						str += '            <div class="col"><span><i class="far fa-trash-alt replyDeleteBtn" title="삭제하기">삭제</i></span></div>';
					}else{
						str += '            <div class="col" style="visibility:hidden"><span><i class="fas fa-edit replyModifyBtn" title="수정하기">수정</i></span></div>	';
						str += '            <div class="col" style="visibility:hidden"><span><i class="far fa-trash-alt replyDeleteBtn" title="삭제하기">삭제</i></span></div>';
					}
					str += '          </div>';
					str += '        </div>';
					str += '      </div>';
					str += '      <div class="row body">';
					str += '        <div class="col fw-5">부모작성자</div>';
					str += '        <div class="col"><textarea name="rcontent" id="rercontent" readonly>'+e.rcontent+'</textarea></div>';
					str += '      </div>';
